Shery.imageEffect("#back", {
  style: 5,
  config: {
    "a": { "value": 1.6, "range": [0, 30] },
    "b": { "value": -0.95, "range": [-1, 1] },
    "zindex": { "value": "-22", "range": [-9999999, 9999999] },
    "aspect": { "value": 0.932798352356951 },
    "ignoreShapeAspect": { "value": true },
    "shapePosition": { "value": { "x": 0, "y": 0 } },
    "shapeScale": { "value": { "x": 0.5, "y": 0.5 } },
    "shapeEdgeSoftness": { "value": 0, "range": [0, 0.5] },
    "shapeRadius": { "value": 0, "range": [0, 2] },
    "currentScroll": { "value": 0 },
    "scrollLerp": { "value": 0.07 },
    "gooey": { "value": true },
    "infiniteGooey": { "value": true },
    "growSize": { "value": 1, "range": [1, 11] },
    "durationOut": { "value": 0.1, "range": [0.1, 5] },
    "durationIn": { "value": 0.81, "range": [0.1, 5] },
    "displaceAmount": { "value": 0.5 },
    "masker": { "value": true },
    "maskVal": { "value": 1.27, "range": [1, 5] },
    "scrollType": { "value": 0 },
    "geoVertex": { "range": [1, 64], "value": 1 },
    "noEffectGooey": { "value": false },
    "onMouse": { "value": 1 }, // Default value
    "noise_speed": { "value": 1.53, "range": [0, 10] },
    "metaball": { "value": 0.21, "range": [0, 2], "_gsap": { "id": 3 } },
    "discard_threshold": { "value": 0.54, "range": [0, 1] },
    "antialias_threshold": { "value": 0, "range": [0, 0.1] },
    "noise_height": { "value": 0.5, "range": [0, 2] },
    "noise_scale": { "value": 12.98, "range": [0, 100] }
  },
  gooey: true
});

// Handle mouse leave from top and bottom
const mainElement = document.querySelector("#main");

mainElement.addEventListener("mouseleave", (event) => {
  const rect = mainElement.getBoundingClientRect();

  // Check if the cursor exits from the top or bottom
  if (event.clientY < rect.top || event.clientY > rect.bottom) {
    console.log("Cursor exited from top or bottom");
    Shery.updateConfig({
      onMouse: { value: 0 } // Set onMouse to 0
    });
  }
});

// Restore the effect when the mouse re-enters
mainElement.addEventListener("mouseenter", () => {
  console.log("Cursor re-entered");
  Shery.updateConfig({
    onMouse: { value: 1 } // Restore onMouse to 1
  });
});

var elems = document.querySelectorAll(".elem");

elems.forEach(function(elem) {
  var h1s = elem.querySelectorAll("h1");
  var index = 0;
  document.querySelector("#main").addEventListener("click", function() {
    gsap.to(h1s[index], {
      top: '-=100%',
      ease: Expo.easeInOut,
      duration: 1,
      onComplete: function(){
        gsap.set(this._targets[0],{top:"100%"} );
      }
    });
    index === h1s.length -1 ? (index=0) :  index++;

    gsap.to(h1s[index], {
      top: '-=100%',
      ease: Expo.easeInOut,
      duration: 1,
    });
  });    
})

